...
//查询并展示 包含修改和删除按钮
function query(url, student){
    
    const data = network(url, student)
    ...
    data.forEach(function(result,index){   
        ...
        contain1.textContent = "学号" + result.sid +"姓名" + result.name
        span.appendChild(contain1)
        const contain2 = document.createElement('div')
        contain2.textContent = "专业" + result.major +"学院" + result.college+"手机号"+result.phone
        span.appendChild(contain2)        
        ...
        const urlchange = './changeS.html?data=' + encodeURIComponent(jsonData);
